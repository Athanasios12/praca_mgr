...
\lstinputlisting[language={C++}, label={lst:viterbi_serial_init}, caption=Inicjalizacja zmiennych, linerange=222-238, firstnumber=222]{viterbi_source/Viterbi.cpp} 

\lstinputlisting[language={C++}, label={lst:viterbi_serial_loop}, caption=Główna pętla programu, linerange=238-291, firstnumber=238]{viterbi_source/Viterbi.cpp} 

%Na razie zostawić

%listing z viterbiLineDetect - nie pokazywac całego
%\lstinputlisting[language={C++}, label={lst:viterbi_serial}, caption=Szeregowa implementacja algorytmu Viterbiego do wykrywania linii, linerange=222-291, firstnumber=222]{viterbi_source/Viterbi.cpp} 
...
Na podstawie wcześniej opisanego algorytmu detekcji linii oraz opisanej szeregowej implementacji można
...
\lstinputlisting[language={C++}, label={lst:viterbi_threads}, caption=Wielowątkowa impleme, linerange=390-423, firstnumber=390]{viterbi_source/Viterbi.cpp} 
W celu właściwego wykorzystania zasobów procesora stosowana jest funkcja \code{std::thread::hardware\_concurrency()} do określenia rekomendowanej liczby jednocześnie uruchomionych wątków.
Cały obraz jest przetwarzany fragmentami po \code{num\_of\_threads} wątków na raz, gdzie dla każdego z nich przypisana jest inna kolumna startowa $y_{start}$(patrz równanie \ref{eq:L_back}). Po tym jak wszystkie wątki wykonają swoje obliczenia wynik jest zapisywany i zaczyna się nowa iteracja, w której rozpatrywany jest następny zestaw kolumn obrazu. Pętla będzie się wykonywać dopóki nie zostaną przetworzone wszystkie kolumny, co reprezentowane jest przez wartość licznika \code{to\_process}.
%cały listing
