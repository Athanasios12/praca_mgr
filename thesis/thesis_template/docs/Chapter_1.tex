...
jeden wynik. Funkcja \code{std::async()} odpowiada za asynchroniczne uruchomienie podanej w liście argumentów funkcji. Zwraca obiekt typu \code{std::future}, z którego
możemy otrzymać wynik używając metody \code{get}. Zaletą tego podejścia jest jego prostota, kiedy nie potrzebujemy skomplikowanej synchronizacji pomiędzy wątkami,
