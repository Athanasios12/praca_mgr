...
\section{Współbieżność dla architektury CPU}
...
\caption{Procesora Intel Core i7, wykorzystujący \textbf{SMT} \protect\cite{OS_Stallings}}
\label{fig:cpu_i7}
\end{figure}
...
równoległych obliczeń. Jedną z metod osiągania współbieżności jest stosowanie architektury \textbf{SIMD} do wykonywania jednej instrukcji dla wielu elementów danych. Dzięki temu możliwe jest dzielenie wykonywania pojedynczej instrukcji programu przez wiele procesorów.\cite{OpenCL_Gaster}\cite{Computer_Architecture_Patterson_Hennesy}

%core simd
\begin{figure}[h]
\includegraphics[scale=1]{simd_core}
...
%cpu vs gpu
\begin{figure}[h]
\includegraphics[scale=1]{gpu_cpu}
\caption{Schemat rdzenia procesora wykorzystującego architekturę SIMD \textbf{SMT} \protect\cite{OS_Stallings}}
\label{fig:gpu_cpu}
\end{figure}
