...
\indent Wielowątkowe podejście do równoległego przetwarzania instrukcji programu
umożliwia wykorzystanie nowoczesnej architektury procesorów, które wyposażone są w kilka rdzeni, każdy wyposażony w własne rejestry, ALU, jednostkę kontrolną oraz dodatkowo w cache L1, czasami rownież L2. Współdzielą szynę pamięci, cache L3 oraz peryferia(patrz rys.\ref{fig:cpu_i7}).\cite{OS_Stallings}
\begin{figure}[h]
\includegraphics[scale=0.7]{cpu_i7}
\caption{Schemat blokowy procesora Intel Core i7 \protect\cite{OS_Stallings}}
\label{fig:cpu_i7}
\end{figure}
%%%%%%%CPU END%%%%%%%%%%%%%
%GPU  i GPU vs CPU
