...
jeden procesor(\textit{Host}) zarządza pracą pozostałych urządzeń(\textit{Device}) wykonujących współbieżne funkcje napisane w OpenCL C, zwane kernelami(\textit{kernels}).
...
\indent%opis co to kernel, work group work item

\indent%opis jak wygląda hierarhia pamięci

\indent%opis jak wygląda typowy flow programu
% + przykładowy skomentowany kod z podziałem na określone wcześniej fazy

\indent%moze podsumowanie i tyle 
\end{document}
